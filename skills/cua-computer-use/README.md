...
## 驱动开发控件（Tauri / Electron / Flutter / …）

Cua **不会**对所有工具包走同一条 CDP。官方限制：

| 运行时 | 通道 |
|--------|------|
| Chrome / Edge / **Electron**（单窗口单页） | CDP `page`，失败再 AX |
| **Tauri / WKWebView / WebView2 / WebKitGTK** | 只有 AX + 像素。typed CDP 会被拒绝 |
| **Flutter** | Semantics → 像素。空树就当 canvas |
| Qt Widgets / GTK / WPF | 当原生 AX。Qt Quick、自绘走像素 |
| Unity / Blender / WebGL | 必须前台 + 像素，后台 click 经常吞掉 |

Agent：`"$CUA" frameworks` 或读 [references/frameworks.md](./references/frameworks.md)。
